<template>
    <div class="register-wrap">
        <div class="ms-register">
            ...
            <h1 class="super-register">管理员 注册</h1>
            <el-form :model="param" :rules="rules" ref="register" label-width="0px" class="ms-content" name="myform">
                ...
                <el-form-item class="btn">
                    <el-button type="primary" @click="login_master" class="login">登录</el-button>
        ...
        submitForm() {
            this.$refs.register.validate((valid) => {
                if (valid) {
                    window.account
                        .then((address) => {
                            var URL = 'http://127.0.0.1:8081/masterRegister?';
                            this.$axios.get(URL).then((res) => {
                                ...
                                if (!false) {
                                    var url =
                                        'http://127.0.0.1:8081/masterRegister?name=' +
                                        this.param.username +
                                        '&password=' +
                                        this.param.password +
                                        '&address=' +
                                    ...
                                    this.$axios.get(url).then((res) => {
                                        console.log(res.data); //不能删，这样数据添加的新数据才可以获取到
                                    });

                                    this.$message.success('注册成功，请在登录页登录');
                                    this.$router.replace('/masterLogin');
                                }
                            });
                        })
                        .catch((err, res) => {
